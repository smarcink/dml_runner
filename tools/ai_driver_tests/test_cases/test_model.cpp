...
inline dnnl::graph::logical_tensor to_onednn_logical_tensor(std::size_t onednn_logical_tensor_id, const ai_driver::Tensor& tensor)
{
...
inline dnnl::graph::tensor onednn_get_or_allocate_graph_mem(const dnnl::graph::logical_tensor& lt, std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>>& data_buffer, const dnnl::engine& eng)
{
...
public:
    ...
    std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>> get_results(const std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>>& input_host_data)
    {
        ...
        std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>> host_data = input_host_data;
        ...
        std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>> return_data{};
    ...
    void set_tensor(std::size_t id, const ai_driver::Tensor& tensor)
    {
...
protected:
    void run_test()
    {
        ...
        auto model = ai_driver::Model(ctx_, stream, md_, inputs_);
    ...
    const std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>>& get_output_data()
    {
        return output_node_id_to_data_;
    }

    const std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>>& get_reference_data()
    {
    ...
    void add_input_mapping_and_data(ai_driver::NodeID port_id, const ai_driver::Tensor& tensor, const std::vector<std::uint8_t>& data)
    {
...
protected:
    ...
    ai_driver::ModelDescriptor md_{};
    ai_driver::TensorMapping inputs_{};
    std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>> input_node_id_to_data_;
    std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>> output_node_id_to_data_;
    std::unordered_map<ai_driver::NodeID, std::vector<std::uint8_t>> references_id_to_data_;
    std::unordered_map<ai_driver::NodeID, typename ContextT::ResourceT> node_id_to_resource_;
...
TEST(ModelTest, ConvPlusAddFusion)
{
    ...
    inputs[input_a] = ai_driver::Tensor(data_type, { 1, 1, 4, 4 });
    inputs[input_b] = ai_driver::Tensor(data_type, { 1, 1, 4, 4 });

    auto model = ai_driver::Model(ctx, stream, md, inputs);
