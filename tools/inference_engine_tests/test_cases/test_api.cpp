...
TEST(ApiTest, get_outputs_single_out)
{
    ...
    ASSERT_TRUE(output_mappings.find(out_node) != std::end(output_mappings));
    ...
    //ASSERT_EQ(inferenceEngineModelGetOutputs(model, nullptr, &outputs_counts), true);
    //ASSERT_EQ(outputs_counts, 1);
    //std::vector<inference_engine_tensor_mapping_t> output_mappings(outputs_counts);
    //ASSERT_EQ(inferenceEngineModelGetOutputs(model, output_mappings.data(), &outputs_counts), true);
