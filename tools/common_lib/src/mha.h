...
namespace gpu_op
{
    ...
    public:
            ...
            :DirectMlBaseNode(dml_device, d3d12_device)
        {
            ...
            {
                const dml::TensorDimensions input_dims_qkv{ shape_input_qkv.n, shape_input_qkv.c, shape_input_qkv.h, shape_input_qkv.d, shape_input_qkv.w };
                const dml::TensorDimensions input_dims_q{ shape_input_q.n, shape_input_q.c, shape_input_q.h };
                const dml::TensorDimensions input_dims_kv{ shape_input_kv.n, shape_input_kv.c, shape_input_kv.h, shape_input_kv.d, shape_input_kv.w };
...
class MhaBaseDispatcher: public NodeDispatcher
{
...

    {
        ...
        {
            input_data_stacked_qkv.resize(get_tensor_elements_count(params_.shape_input_qkv, params_.layout) * get_data_type_bytes_width(params_.dt));
        ...
        {
            input_data_stacked_q.resize(get_tensor_elements_count(params_.shape_input_q, DataLayout::eNCH) * get_data_type_bytes_width(params_.dt));
            input_data_stacked_kv.resize(get_tensor_elements_count(params_.shape_input_kv, params_.layout) * get_data_type_bytes_width(params_.dt));
        ...
        const auto tensor_out_bytes_width = get_tensor_elements_count(out_shape, DataLayout::eNHW) * get_data_type_bytes_width(params_.dt);
    ...
    std::uint32_t get_width() const
    {
        ...
        if (params_.type == MhaType::MhaType_QKV)
        {
            return params_.shape_input_qkv.h * params_.shape_input_qkv.w;
        }
        if (params_.type == MhaType::MhaType_Q_KV)
        {
            return params_.shape_input_q.h;
        }
        assert(false && "Not supported");
        return 0;
    }
