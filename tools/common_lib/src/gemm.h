...
public:
        ...
        , dnnl_engine_(dnnl::iumd_interop::make_engine(&device_))
    {
        ...
        if (params_.b_managed)
        {
            dnnl::reorder::primitive_desc reorder_desc(dnnl_engine_, to_dnnl_mem_desc(params_.shape_b, params_.layout, params_.quant_dt), dnnl_engine_, input_b_memory_desc_);
            reorder_input_b_ = dnnl::reorder(reorder_desc);
        }

        if (params_.bs_managed)
        {
            dnnl::reorder::primitive_desc reorder_desc(dnnl_engine_, to_dnnl_mem_desc(shape_bsz, params_.layout, params_.dt), dnnl_engine_, input_scales_memory_desc_);
            reorder_input_scales_ = dnnl::reorder(reorder_desc);
        }

        if (params_.bz_managed)
        {
            dnnl::reorder::primitive_desc reorder_desc(dnnl_engine_, to_dnnl_mem_desc(shape_bsz, params_.layout, params_.quant_dt), dnnl_engine_, input_zeropoints_memory_desc_.value());
            reorder_input_zeropoints_ = dnnl::reorder(reorder_desc);
        }
    ...
    void initialize(ID3D12GraphicsCommandList* cmd_list, D3D12_CPU_DESCRIPTOR_HANDLE cpu_handle, D3D12_GPU_DESCRIPTOR_HANDLE gpu_handle) override
    {
        ...
        if (reorder_input_scales_)
        {
            ...
            dnnl::memory input_memory = create_dnnl_memory(input_memory_desc_, umd_input_mem);
            dnnl::memory reorder_memory = create_dnnl_memory(input_scales_memory_desc_, umd_persistent_mem, persistent_mem_offset);
        ...
        if (reorder_input_zeropoints_)
        {
            ...
            dnnl::memory input_memory = create_dnnl_memory(input_memory_desc_, umd_input_mem);
            dnnl::memory reorder_memory = create_dnnl_memory(input_zeropoints_memory_desc_.value(), umd_persistent_mem, persistent_mem_offset);
    ...
    void execute(ID3D12GraphicsCommandList* cmd_list) override
    {
        ...
        dnnl::memory input_scales_memory = [&](std::size_t& persistent_mem_offset)
        {
            ...
            return create_dnnl_memory(input_scales_memory_desc_, umd_input_scales_memory_, reorder_input_scales_ ? offset : 0ull);
        }(persistent_mem_offset);
        ...
        dnnl::memory input_zeropoints_memory = [&](std::size_t& persistent_mem_offset)
        {
            if (params_.has_zero_point)
            {
                ...
                return create_dnnl_memory(input_zeropoints_memory_desc_.value(), umd_input_zeropoints_memory_, reorder_input_zeropoints_ ? offset : 0ull);
            }
            return dnnl::memory{};
        }(persistent_mem_offset);
