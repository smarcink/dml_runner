...
namespace ai_driver
{
    ...
    inline ai_driver_context_callbacks_t G_GPU_CBCS = {};
    class GpuKernel;
    ...
    public:
        GpuResource() = default;
        GpuResource(ai_driver_resource_t r) 
            : handle_(r)
            , is_owner_(false)
        {
            std::cout << "GpuResource C-tor non-owning handle" << std::endl;
        }
        GpuResource(ai_driver_device_t device, std::size_t size)
        ...
        ai_driver_resource_t get() { return handle_; }
        const ai_driver_resource_t get() const { return handle_; }

    protected:
        ai_driver_resource_t handle_ = nullptr;
    ...
    public:
        GpuStream(ai_driver_stream_t stream)
            : handle_(stream)
        {
        ...
        ai_driver_stream_t get() { return handle_; }
    ...
    public:
        GpuKernel() = default;
        GpuKernel(ai_driver_device_t device, const char* kernel_name, const void* kernel_code, std::size_t kernel_code_size, const char* build_options, ai_driver_kernel_language_t language)
        ...
        ai_driver_kernel_t get() { return handle_; }
        ai_driver_kernel_t get() const { return handle_; }
    ...
    private:
        ai_driver_kernel_t handle_ = nullptr;
    };

    class GpuContext
    {
    public:
        GpuContext(ai_driver_device_t device, ai_driver_context_callbacks_t callbacks)
            : device_(device)
        {
        ...
        GpuKernel::Ptr create_kernel(const char* kernel_name,
            const void* kernel_code, std::size_t kernel_code_size, const char* build_options, ai_driver_kernel_language_t language)
        {
    ...
    private:
        ai_driver_device_t device_ = nullptr;
    };
