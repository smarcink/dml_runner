...
namespace ai_driver
{
    ...
    */
    ...
    {
    public:
        INode(std::size_t id, const std::vector<ai_driver_node_id_t>& inputs, std::string_view name)
            : id_(id)
            , inputs_(inputs)
            , name_(name)
        {
            assert(id != AI_DRIVER_INVALID_NODE_ID);
        }
        ...
        virtual const std::vector<ai_driver_node_id_t>& inputs() const {
            return inputs_;
        }

        virtual std::unique_ptr<GpuNode> create_gpu_node(const std::vector<GpuNode*>& inputs) = 0;
    protected:
        std::size_t id_ = AI_DRIVER_INVALID_NODE_ID;
        std::vector<ai_driver_node_id_t> inputs_{};
        std::string name_;
    };
    ...
    struct PostOp
    {
        struct ElemWisePosOp {
            ai_driver_elementwise_desc_t desc;
        ...
        std::variant<ai_driver_activation_desc_t, ElemWisePosOp> params_;
    };
    ...
    */
    ...
    protected:
        std::size_t id_ = AI_DRIVER_INVALID_NODE_ID; // init with invalid id
